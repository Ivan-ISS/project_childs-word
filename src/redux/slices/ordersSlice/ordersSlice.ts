...
>('submitCart/fetch', async (_, thunkAPI) => {
    try {
...
export const fetchOrders = createAsyncThunk<
    ...
    FetchOrdersArgs,
    { rejectValue: FetchSubmitCartError | undefined }
>('orders/fetch', async (args, thunkAPI) => {
    try {
        const response = await fetch(
            `${routes.urlGetOrders()}?page=${args.page}&limit=${args.limit}`,
            {
                method: 'GET',
                credentials: 'include',
            }
        ...
        builder
            ...
            .addCase(fetchSubmitCart.fulfilled, (state, action: PayloadAction<IOrderInfo[]>) => {
                ...
                if (
                    ...
                    Math.ceil(state.params.totalOrders / state.params.limit)
                ) {
                    state.orders = [...state.orders, action.payload];
                }
            })
            .addCase(
                fetchSubmitCart.rejected,
                (state, action: PayloadAction<FetchSubmitCartError | undefined>) => {
            ...
            .addCase(
                ...
                (state, action: PayloadAction<FetchOrdersError | undefined>) => {
                    state.status = 'download failed';
                    if (action.payload) {
                        state.error = action.payload.message;
                    }
